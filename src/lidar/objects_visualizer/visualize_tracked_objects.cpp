...
{
  label_color_=ParseColor({255.,255.,255.,1.0});
  arrow_color_=ParseColor({0.,255.,0.,0.8});
  hull_color_=ParseColor({51.,204.,51.,0.8});
  box_color_=ParseColor({51.,128.,204.,0.8});
  model_color_=ParseColor({190.,190.,190.,0.5});
  centroid_color_=ParseColor({77.,121.,255.,0.8});
...
bool VisualizeDetectedObjects::Init(Json::Value params, std::string key){
  //std::cout<<"params"<<params<<std::endl;
    if(params.isMember(key)) {
        Json::Value visualization_param = params[key];

        if (visualization_param.isMember("object_speed_threshold") && visualization_param["object_speed_threshold"].isInt()) {
            object_speed_threshold_ = visualization_param["object_speed_threshold"].asInt();
        } else {
            logger.Log(ERROR, "[%s]: Has not key named object_speed_threshold in the tracker config.\n", __func__);
            return false;
        }
        if (visualization_param.isMember("arrow_speed_threshold") && visualization_param["arrow_speed_threshold"].isDouble()) {
            arrow_speed_threshold_ = visualization_param["arrow_speed_threshold"].asFloat();
        } else {
            logger.Log(ERROR, "[%s]: Has not key named arrow_speed_threshold in the tracker config.\n", __func__);
            return false;
        }
        if (visualization_param.isMember("marker_display_duration") && visualization_param["marker_display_duration"].isDouble()) {
            marker_display_duration_ = visualization_param["marker_display_duration"].asFloat();
        } else {
            logger.Log(ERROR, "[%s]: Has not key named marker_display_duration in the tracker config.\n", __func__);
            return false;
        }
    }else{
        logger.Log(ERROR, "[%s]: Has not key named visualization in the perception config.\n", __func__);
    }
}
...
VisualizeDetectedObjects::ObjectsToCentroids(const perception_ros::DetectedObjectArray &in_objects)
{
...
visualization_msgs::MarkerArray
VisualizeDetectedObjects::ObjectsToBoxes(const perception_ros::DetectedObjectArray &in_objects)
{
  ...
  for (auto const &object: in_objects.objects)
  {
    ...
    //   if (object.pose_reliable)//pose is not reliable
    //   std::cout<<"object.pose_reliable:"<<object.pose_reliable<<std::endl;
...
visualization_msgs::MarkerArray
VisualizeDetectedObjects::ObjectsToModels(const perception_ros::DetectedObjectArray &in_objects)
{
...
visualization_msgs::MarkerArray
VisualizeDetectedObjects::ObjectsToHulls(const perception_ros::DetectedObjectArray &in_objects)
{
...
visualization_msgs::MarkerArray
VisualizeDetectedObjects::ObjectsToArrows(const perception_ros::DetectedObjectArray &in_objects)
{
...
visualization_msgs::MarkerArray
VisualizeDetectedObjects::ObjectsToLabels(const perception_ros::DetectedObjectArray &in_objects)
{
...
bool VisualizeDetectedObjects::IsObjectValid(const perception_ros::DetectedObject &in_object)
{
  if (!in_object.valid ||
