  ...
  , std_lane_direction_(0.15)//这个是没有用的
{
  ...
  std_a_cv_ = 1.5; //acceleration noise
  std_a_ctrv_ = 1.5;
  std_a_rm_ = 3;
  std_ctrv_yawdd_ = 1.5; //yaw acceleration???
  std_cv_yawdd_ = 1.5;
  std_rm_yawdd_ = 3;
...
{//the yaw angle is restricted between -M_PI:M_PI
...
void UKF::updateMeasurementForCTRV(const std::vector<perception_ros::DetectedObject>& object_vec)
{
...
void UKF::updateSUKF(const std::vector<perception_ros::DetectedObject>& object_vec)
{
...
double UKF::calculateNIS(const perception_ros::DetectedObject& in_object, const int motion_ind)
{
...
bool UKF::isLaneDirectionAvailable(const perception_ros::DetectedObject& in_object, const int motion_ind,
...
void UKF::checkLaneDirectionAvailability(const perception_ros::DetectedObject& in_object,
